...
export default function App(): JSX.Element {
  ...
  const sinceRef = useRef<number>(Math.floor(Date.now() / 1000) - 60);
    ...
    async function poll(): Promise<void> {
      try {
        const res = await fetch(
          `${API_URL}/api/reports?sinceTs=${sinceRef.current}&limit=100`,
        );
        if (!res.ok) throw new Error(String(res.status));
        const json = (await res.json()) as { items: Report[]; serverTs: number };
        setConnected(true);
        for (const r of json.items.reverse()) push(r);
        sinceRef.current = json.serverTs;
      } catch {
        setConnected(false);
      }
    }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-chrome-600">
                Live Inspection & Reporting · polling every {POLL_MS / 1000}s
              </div>
            </div>
          </div>
      ...
      <main className="p-6">
        {reports.length === 0 ? (
          <div className="rounded border border-dashed border-carbon-700 p-10 text-center text-chrome-600">
            รอ report จาก backend... (cron ทุก 1 นาที หรือกด{' '}
            <code className="text-ice-400">/api/tick</code> เพื่อยิงทันที)
          </div>
        ) : (
